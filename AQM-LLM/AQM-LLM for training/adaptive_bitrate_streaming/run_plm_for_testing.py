...
def run(args):
    ...
    # 强制定义 model_dir，确保包含 ._ss_None
    if args.model_dir is None:
        # 在路径中插入 ._ss_None，如果必要
        args.model_dir = f"/workspace/NetLLM/adaptive_bitrate_streaming/data/ft_plms/llama_base/._ss_None/rank_{args.rank}_w_{args.w}_gamma_{args.gamma}_sfd_{args.state_feature_dim}_lr_{args.lr}_wd_{args.weight_decay}_warm_{args.warmup_steps}_epochs_{args.num_epochs}_seed_{args.seed}/early_stop_{args.which_layer}_best_model"
    
    print(f"Model directory: {args.model_dir}")
    
    # 检查路径是否存在
    assert os.path.exists(args.model_dir), f"Model weight dir {args.model_dir} does not exist."
